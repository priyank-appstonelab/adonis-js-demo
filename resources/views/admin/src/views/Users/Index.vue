      ...
      <div>
        ...
        >
          <template slot="sn" slot-scope="props">
            <router-link
              :to="{ name: 'UsersEdit', params: { id: props.rowData.id } }"
          ...
          <template slot="actions" slot-scope="props">
            <div class="dropdown inline relative">
              ...
              <ul class="dropdown-menu hidden text-gray-700 pt-1">
                <a
                  v-if="$auth.user().id != props.rowData.id"
                  href="#"
...
export default {
  ...
  data() {
    return {
      ...
      api: this.$baseApi + `/users`,
      columns: [
        {
          name: "id",
          title: () => {
            return this.$t("users.column.id");
          },
          active: false,
          ref: "id",
        },
...

        {
          name: "name",
          title: () => {
            return this.$t("users.column.name");
          },
          active: true,
...

        {
          name: "email",
          title: () => {
            return this.$t("users.column.email");
          },
          active: true,
...

        {
          name: "avatar",
          title: () => {
            return this.$t("users.column.avatar");
          },
          active: true,
          callback: (s) => {
            if (s?.url)
              return `<img class="border shadow h-10 w-10" alt="" src"${s.url}" >`;
          },
        },

        {
          name: "created_at",
          title: () => {
            return this.$t("users.column.created_at");
          },
          active: true,
...

        {
          name: "updated_at",
          title: () => {
            return this.$t("users.column.updated_at");
          },
          active: true,
  ...
  methods: {
    ...
    goto(e) {
      this.$router.push({ name: "UsersEdit", params: { id: e.id } });
    },
    loadTable() {
      ...
      this.$store
        ...
        .then((data) => {
          handleSuccess(
            this,
            data.message ||
              this.$t(`success.delete`, { resource: `User #${id}` })
          );
          this.refreshTable();
        })
        .catch((err) => {
          handleError(
            this,
            err,
            this.$t(`error.delete`, { resource: `User #${id}` })
          );
        })
        .finally(() => {
          this.model = {};
    ...
    confirmDeletion(model) {
      this.title = this.$t("delete.title", { resource: `User #${id}` });
      this.description = this.$t("delete.description", {
        resource: `User #${id}`,
      });
      this.model = model;
