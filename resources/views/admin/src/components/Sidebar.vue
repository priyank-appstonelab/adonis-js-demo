<template>
  <div class="flex">
    ...
    <div
          :class="`items-center duration-200 mt-4 py-2 px-6 border-l-4 ${$route.path.includes('users') ? activeClass : inactiveClass}`"
          @click="toggleExpandMenu('users')"
          > <i class="icon-expand-alt"></i> Users
          <ul v-if="expandedMenu == 'users'" class="p-3">
        <li class="p-2"><router-link

        :class="$route.name == 'UsersIndex' ? activeChildClass : inactiveChildClass"
          :to="{name: 'UsersIndex'}"
        >
       ...
       <li class="p-2"> <router-link
          ...
          :class="$route.name == 'UsersNew' ? activeChildClass : inactiveChildClass"
          :to="{name: 'UsersNew'}"
        >
